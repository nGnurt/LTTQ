...
namespace LTTQ
{
    public partial class dangky : Form
    {
        ...
        private void btndangky_Click(object sender, EventArgs e)
        {
            if(check()==1)
            {
                if(ketnoi.loaddulieu("select * from QuanLiNhanVien where TaiKhoan='"+txtdangkyemail.Text+"' or email='"+txtdangkyrepass+"'")==null|| ketnoi.loaddulieu("select * from dangky where user='" + txtdangkyemail.Text + "' or email='" + txtdangkyrepass + "'")==null)
                {
                    MessageBox.Show("Tài khoản hoặc mật khẩu đã có rồi", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                } 
                else
                {
                    ketnoi.CapNhatDuLieu("Insert INTO dangky values('" + txtdangkyemail.Text + "','" + txtdangkypass.Text + "','" + txtdangkyrepass.Text + "')");
                }    
            }    
        }
